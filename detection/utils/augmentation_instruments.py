...
def save_image(image, path, name, suffix=""):
    ...
    """
    if not cv2.imwrite(os.path.join(path, name) + suffix + ".jpg", image):
        raise Exception("Could not write image")


class PictureAugmentation(object):
    """
...
class YOLORotateBbox(object):
    ...
    @staticmethod
    def yolo_format_to_cv(x1, y1, x2, y2, h, w):
        """
        ...
        voc = [center_x - (bbox_width / 2), center_y - (bbox_height / 2), center_x + (bbox_width / 2),
               center_y + (bbox_height / 2)]

        return [int(v) for v in voc]

    @staticmethod
    def cv_format_to_yolo(corner, h, w):
        """
        ...
        return corner[0], round(center_bbox_x / w, 6), round(center_bbox_y / h, 6), round(bbox_w / w, 6), round(
            bbox_h / h,
            6)

    def rotate_yolo_bbox(self):
        ...
        for x in f1:
            bbox = x.strip('\n').split(' ')
            if len(bbox) > 1:
                (center_x, center_y, bbox_width, bbox_height) = self.yolo_format_to_cv(float(bbox[1]), float(bbox[2]),
